...
from .forms import MP3Form, MP3FinnForm
...
@api_view(['POST', 'GET'])
def create_mp3_finn(request):
    max_length = None
    if request.method == 'POST':
        form = MP3FinnForm(request.POST)
        sentences = []
        if form.is_valid():
            for i in range(1,11):
                sentence = form.cleaned_data[f"words{i}"]
                if sentence != "":
                    counter = form.cleaned_data[f"count{i}"]
        ...
        form = MP3FinnForm()
        max_length = form.fields['words1'].max_length

    return render(request, 'finn.html', {'form': form, 'max_length': max_length})
